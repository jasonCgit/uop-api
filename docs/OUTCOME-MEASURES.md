...
│  Section 5 │ SRE Coverage — Baseline Roll-up        [NEW]   │
├─────────────────────────────────────────────────────────────┤
...
|--------|-----------|-------------|--------|
| Prompt Executions | Volume of AI-assisted code interactions | Count of prompt executions per period | Area chart, monthly trend |

### 1.2 UOP Platform

| Metric | Definition | Calculation | Visual |
|--------|-----------|-------------|--------|
| Chat Volume | AURA chat interactions within UOP | Count of user prompts to AURA per period | Area chart, monthly trend |
| Click Volume | User engagement with UOP dashboards | Count of meaningful clicks/actions per period | Area chart, monthly trend |
...
### 1.4 Per-CTO/LOB Segmentation & Leaderboards [NEW]

| Metric | Definition | Calculation | Visual |
|--------|-----------|-------------|--------|
| Usage by Segment | Adoption volumes broken by CTO/LOB/app owner | Sum of events per segment over selected period | Leaderboard table with sparklines |
| Share Mix | Proportional usage across segments | Percentage of total per CTO/LOB | Stacked 100% bar or treemap |
...
│  └────────────────────────────────────────────────────────────┘ │
├─────────────────────────────────────────────────────────────────┤
│  LEADERBOARD — Per CTO/LOB                                      │
│  ┌────────────────────┬──────┬──────┬──────┬──────────────────┐ │
│  │ CTO / LOB          │ AIOps│  UOP │  MCP │ Trend            │ │
│  ├────────────────────┼──────┼──────┼──────┼──────────────────┤ │
...
| Dynatrace & Golden Signal Coverage | Coverage | Uplift on Day 1 deliverables | % onboarded vs target; trend |
...
|--------|------|-----------|-------------|
| Prompt Executions | Usage | Volume in DevGPT against Zero-Touch M&O MCP | Count per period |
| UOP Onboarding & SLOs | Coverage | Uplift on Day 1 deliverables | Apps onboarded vs target; trend |
...
| Metric | Type | Definition | Calculation |
|--------|------|-----------|-------------|
| Component Mappings | Productivity | Dependency mappings & REST calls tracked in knowledge graph | One-time count + incremental adds |
...
| Metric | Type | Definition | Calculation |
|--------|------|-----------|-------------|
| Open Incident Duration | Effectiveness | Track open incident duration vs baseline | RCA summaries into RR-created incidents; compare against avg baseline |
...
| AURA Noise Filtering Rate | Effectiveness [NEW] | Suppression rate = suppressed / total raw alerts | Count of alerts suppressed by AURA ÷ raw alert count per period; report by app and aggregate |
| UOP Precision Trend | Effectiveness [NEW] | % of UOP alerts that lead to action (click, AIOps inquiry, incident linkage) | Alerts with any correlated action ÷ total UOP alerts per period |
| SRE Toil Reduction | Operational [NEW] | Actionable alerts per on-call per week + after-hours pages per week | Count of actionable alerts/pages allocated to on-call schedules |
...
| Change-Aware Anomaly Rate | Effectiveness [NEW] | Anomalies per 100 changes — contextualizes stability vs pace of change | (Total anomalies ÷ total code changes or deployments in same period) × 100 |
...
│  │ Toil     -2.4/wk  │  │ Change Rate  3.2%  │                 │
│  │ Noise Filt.  84%   │  │ Incidents     -18  │                 │
│  └────────────────────┘  └────────────────────┘                 │
│                                                                 │
│  ┌─ Blast Radius ────┐  ┌─ FinOps ───────────┐                 │
│  │ Mappings   2,847   │  │ Identified  $1.2M  │                 │
│  │ REST Calls 4,120   │  │ Executed     $840k │                 │
│  └────────────────────┘  └────────────────────┘                 │
│                                                                 │
│  ALERT SEVERITY — NOISE REDUCTION TREND                         │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │ Raw Alerts   ████████████████████████████████  12,400      │ │
│  │ After AURA   ████████████░░░░░░░░░░░░░░░░░░░   4,200      │ │
│  │ Actioned     ██████░░░░░░░░░░░░░░░░░░░░░░░░░   2,100      │ │
...
│  └────────────────────────────────────────────────────────────┘ │
│                                                                 │
│  AURA — CHANGE-AWARE ANOMALY RATE                               │
│  ┌────────────────────────────────────────────────────────────┐ │
│  │  Anomalies                                                 │ │
│  │  per 100     5 │     ╲                                     │ │
│  │  changes     4 │      ╲___                                 │ │
...
> Better than manual MTTR — this is measured automatically from signal data.

**Definition:** Duration from when a signal state turns RED/AMBER until it returns to GREEN. Measured per signal, reported as distribution bands and medians.
...
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Segmented by: [All Apps ▾]  [All CTOs ▾]  [All Severities ▾]  │
└─────────────────────────────────────────────────────────────────┘
```
...
| Incident Zero | Signal has a corresponding Incident Zero manual entry by SRE | % of signals with matching Incident Zero record; median time from signal to entry |

#### 3.2a Time from Signal to First Action (Latency to Engagement) [NEW]

**Definition:** Time from signal appearance to first user action (UOP click) or first AIOps/IDE inquiry.

**Reporting:** Medians and bands (% within 5m / 30m / 2h / 8h), segmented by app and CTO.
...
#### 3.2b Incident Zero Correlation [NEW]
...
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  Segmented by: [All Apps ▾]  [All CTOs ▾]                       │
└─────────────────────────────────────────────────────────────────┘
```

### 3.3 Who Is Using Our Tools?

Tracks unique users and sessions by tool, segmented by CTO/LOB/app owner.
...
**Per-CTO/LOB Segmentation** [ENHANCED]: Show adoption and active-use rates. Funnel view: awareness → trial → active use.

**Visual — Tool Adoption Funnel:**

```
┌─────────────────────────────────────────────────────────────────┐
│  TOOL ADOPTION — Per CTO/LOB                                    │
│                                                                 │
│  AWM                                                            │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │  Aware (onboarded)  ████████████████████████████████  142│   │
...
**Calculation:** Time series by P-level (P1/P2/P3/P4/P5 and ESD requests). Include normalized rate per 100 services if needed.

**What to show:**
- Baseline on incidence for 2025 for all SEALs onboarded to AIOps360
- Reduction in P1/P2 count tells the proactive prevention story
- Reduction in overall service ticket count tells the TechSupport agent scope story

### 4.2 MTTR by Severity

**Definition:** Mean time to resolve per severity, per period.

**Calculation:** Average incident resolution time by severity; show distribution and medians.
...
**Calculation:** Count of JIRAs with `AIOps360 triggered` label per period. Link anomalies to PRs/fixes.

**Action needed:** SRE leads need these numbers so they can drive this metric up. Bhargav to confirm data source with Eric.
...
**Segmentation:** By CTO, LOB, app owner.
...
**Segmentation:** By CTO/LOB; show gaps and target dates.

### 5.3 UOP Coverage

**Definition:** % of pilot apps with UOP signals and SLOs configured and active.

**Calculation:** (Apps with UOP signals & SLOs / total pilot apps) × 100.

**Segmentation:** By CTO/LOB; include SLOs defined vs. in-progress.
...
│  ┌─ Coverage Summary ───────────────────────────────────────┐   │
│  │                                                          │   │
│  │         Dynatrace          SRE Telemetry      UOP        │   │
│  │        ┌────────┐          ┌────────┐      ┌────────┐    │   │
│  │        │        │          │        │      │        │    │   │
│  │        │  78%   │          │  64%   │      │  52%   │    │   │
│  │        │        │          │        │      │        │    │   │
│  │        │ 142/182│          │ 116/182│      │ 95/182 │    │   │
│  │        └────────┘          └────────┘      └────────┘    │   │
│  │        ▲ 12% QoQ          ▲ 18% QoQ       ▲ 22% QoQ    │   │
│  │                                                          │   │
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ Coverage by CTO/LOB ───────────────────────────────────┐    │
│  │                                                          │   │
│  │  CTO/LOB         │ Dyna  │ Telem │  UOP  │  Gap         │   │
│  │  ─────────────────┼───────┼───────┼───────┼──────        │   │
│  │  AWM              │  88%  │  72%  │  68%  │  12 apps     │   │
│  │  Technology       │  82%  │  70%  │  58%  │  18 apps     │   │
│  │  Global Banking   │  74%  │  58%  │  42%  │  24 apps     │   │
│  │  Consumer         │  62%  │  48%  │  34%  │  31 apps     │   │
│  │                                                          │   │
│  │  Cells color-coded: ■ >75% green  ■ 50-75% amber  ■ <50% red│
│  └──────────────────────────────────────────────────────────┘   │
│                                                                 │
│  ┌─ Coverage Trend ─────────────────────────────────────────┐   │
│  │                                                          │   │
...
### MCP/AURA Use Case Narratives with Measured Outcomes [NEW]
...
### Expanded Per-CTO Dashboards [NEW]

Leaderboards and funnels (awareness → trial → active use) by tool and workstream. Each CTO gets a dedicated view showing their portfolio's coverage, effectiveness, and usage trends.
...
3. **Trend over snapshot.** Every metric should be a trend graph wherever possible. A single number is a data point; a trend is a narrative.

4. **Segment everything by CTO/LOB.** Leadership wants to see their portfolio, not just aggregate numbers. Leaderboards drive healthy competition.
...
| SRE Effectiveness | Atziri, Souhel, Eric V | UOP signal data, AIOpsmode logs, Incident Zero |
| Traditional Support | Atziri, Souhel, Eric V | Incident management system, JIRA |
| SRE Coverage | Atziri, Souhel, Eric V | Dynatrace, SRE telemetry pipelines, UOP config |

---

## Next Steps

1. **Data validation:** Confirm data sources exist for each metric — Mike should have AURA/MCP data per CTO
2. **Baseline establishment:** Pull 2025 baselines for incidents, MTTR, signal counts for all onboarded SEALs
3. **JIRA labeling:** Ensure `AIOps360 triggered` label is being applied to relevant JIRAs
4. **Wireframes:** Use the visuals above as the basis for UOP dashboard wireframes
5. **Day 2 narratives:** Begin collecting before/after stories from each CTO for the first quarterly narrative report
